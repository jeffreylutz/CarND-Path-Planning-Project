...
double PathPlanner::getSpeedChange(bool increase) {
    if (!increase) {
        return ego_speed > SPEED_CHANGE ? -1.0 * SPEED_CHANGE : -0.5 * ego_speed;
    }
    return ego_speed < (SPEED_LIMIT - SPEED_CHANGE) ? SPEED_CHANGE : 0.0;
}
...
bool PathPlanner::attemptLeftLaneChange() {
    if ((ego_state == GO_LEFT || ego_state == GO_EITHER) && hasSafeDistanceToCarsLeftLane()) {
        setLaneChangeTime();
...
bool PathPlanner::attemptRightLaneChange() {
    if ((ego_state == GO_RIGHT || ego_state == GO_EITHER) && hasSafeDistanceToCarsRightLane()) {
        setLaneChangeTime();
        ego_lane = ego_state == GO_LEFT ? ego_lane - 1 : ego_lane + 1;
...
void PathPlanner::update_position(double x, double y, double s, double yaw, double speed) {
    ...
    ego_current_y = y;
    ego_s = s;
    // Only update yaw if speed is greater than 1 mph
    if (speed > 1.0) {
        ego_yaw_deg = yaw;
        ego_yaw_rad = deg2rad(ego_yaw_deg);
    }
    ego_speed = speed;
}
...
void PathPlanner::update_state(const vector<double> &previous_path_x, const double &end_path_s,
    ...
    } else {//else set ego_future_s to current ego_s position
        ego_future_s = ego_s;
    }
...
PathPlanner::realize_state(const vector<double> &previous_path_x, const vector<double> &previous_path_y,
    ...
    emergencyStop = cars_dist_front[ego_lane] < EMERGENCY_BRAKE_DISTANCE;
...
//        cout << msg << " " << getLastLaneChangeDiff() << endl;
    ...
    if (emergencyStop) {
        cout << "EMERGENCY STOP: " << ego_speed << endl;
        ref_v -= getSpeedChange(false);
        ref_v = (ref_v < 0.4) ? 0.0 : ref_v;
        ego_state = GO_STRAIGHT;
    } else if (ego_speed < (SPEED_LIMIT - SPEED_CHANGE) && ref_v < (SPEED_LIMIT - SPEED_CHANGE)) {
//        ref_v += getSpeedChange(!tooCloseToFrontCar);
        ref_v = ego_speed + SPEED_CHANGE;
    }
    ...
    *************************/
    ...
    double ego_last_x = ego_current_x;
    double ego_last_y = ego_current_y;

    //if prev path is almost empty, use the car position as starting reference
    if (prev_size < 2) {
        double ego_x_prev = ego_current_x - cos(ego_yaw_rad);
        double ego_y_prev = ego_current_y - sin(ego_yaw_rad);
        ptsx.push_back(ego_x_prev);
        ptsx.push_back(ego_current_x);
        ptsy.push_back(ego_y_prev);
        ptsy.push_back(ego_current_y);
    } else {//if prev path is not empty, use previous path endpoint as starting point
        ego_last_x = previous_path_x[prev_size - 1];
        ego_last_y = previous_path_y[prev_size - 1];
        double ego_last_prev_x = previous_path_x[prev_size - 2];
        double ego_last_prev_y = previous_path_y[prev_size - 2];
        ego_yaw_rad = atan2(ego_last_y - ego_last_prev_y, ego_last_x - ego_last_prev_x);
        ptsx.push_back(ego_last_prev_x);
        ptsx.push_back(ego_last_x);
        ptsy.push_back(ego_last_prev_y);
        ptsy.push_back(ego_last_y);
    }

//     //   changing from map space to car space by using rotation matrix for each waypoints
//        https://en.wikipedia.org/wiki/Rotation_matrix
    for (int i = 30; i <= 90; i += 30) {
        vector<double> next = getXY(ego_future_s + i, (2 + 4 * ego_lane), map_waypoints_s, map_waypoints_x,
                                    map_waypoints_y);

        // skip adding an element if the x value is not greater than previous x value
        if (!ptsx.empty() && !(next[0] > ptsx.back()) && next[0] - ptsx.back() < 1.0) {
            continue;
        }
    ...
    for (int i = 0; i < ptsx.size(); i++) {
        double shift_x = ptsx[i] - ego_last_x;
        double shift_y = ptsy[i] - ego_last_y;
        ptsx[i] = (shift_x * cos(0 - ego_yaw_rad) - shift_y * sin(0 - ego_yaw_rad));
        ptsy[i] = (shift_x * sin(0 - ego_yaw_rad) + shift_y * cos(0 - ego_yaw_rad));
    }
    ...
    // target_x is the final X value after 5 0 steps.
    // So target_x = 20 ms * speed (mph) / 2.237 -> meters
    double target_x = 0.02 * ref_v / MPS_TO_MPH;
    double target_y = spline(target_x);
    double target_dist = sqrt((target_x) * (target_x) + (target_y) * (target_y)); //assume straight line

    double x_add_on = 0.0;
    double x_point, y_point, x_ref, y_ref, N;

    //top up the rest of the planner next_x_vals and next_y_vals with new planned points
    for (int i = 1; i <= 10 - next_x_vals.size(); i++) {

        //find new x_point and y_point
        N = (target_dist / (.02 * ref_v / MPS_TO_MPH));
        ...
        //x_ref and y_ref represents the next point after the car'spline current location
        x_ref = x_point;
        y_ref = y_point;

        //rotate from car space back to map space but without translation
        //i.e. still taking car as centre (0,o)
        x_point = (x_ref * cos(ego_yaw_rad) - y_ref * sin(ego_yaw_rad));
        y_point = (x_ref * sin(ego_yaw_rad) + y_ref * cos(ego_yaw_rad));

        //convert x_point and y_point with translation to account for car'spline position on map
        x_point += ego_last_x;
        y_point += ego_last_y;

        cout << "ego_speed - ref_v " << pad(ego_speed, 4, 5) << " - " << pad(ref_v, 4, 5) << endl;
